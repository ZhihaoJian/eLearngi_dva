...
class RegisterPage extends React.Component {
    ...
    onSubmit = () => {
        ...
        dispatch({
            ...
            payload: {
                ...formData,
                callback: () => this.setState({ formIsValidate: false })
            }
        })
    }
        ...
        if (value) {
            const fn = () => {
                dispatch({
                    ...
                    payload: { data: value, callback }
                })
            }
            setTimeout(debounce(fn, 500), 0);
        } else {
            callback();
        }
    }
                    ...
                    <Form layout='horizontal' style={{ marginTop: 30 }} >
                        ...
                        <FormItem>
                            <InputGroup>
                                ...
                                <Col span={7} >
                                    <div onClick={this.switchCaptcha} style={{ cursor: 'pointer' }} title='点击更换验证码' >
                                        <img src='/authority/verifyCode' alt='captcha' />
                                    </div>
                                </Col>
                            </InputGroup>
                        </FormItem>
                        ...
                        <FormItem>
                            ...
                            <Col span={12}>
                                <a style={{ float: 'right' }} href='/login' >使用已有账号登录</a>
                            </Col>
                        </FormItem>
                    </Form>
                </div>
...
const RegisterFormPage = Form.create()(RegisterPage);

function mapStateToProps(state) {
    return { ...state.login }
}

export default connect(mapStateToProps)(RegisterFormPage);